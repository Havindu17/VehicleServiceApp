          ...
          {item.status === 'completed' && !item.feedbackGiven && (
            <TouchableOpacity style={styles.feedbackBtn}
              onPress={() => navigation.navigate('CustomerFeedback',
                { bookingId: item._id, garageName: item.garageName })}>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.summaryRow}>
        {[
          { label: 'Total',     value: history.length, color: COLORS.gold    },
          { label: 'Completed', value: completed,       color: COLORS.success },
          { label: 'Active',    value: history.filter(h =>
              h.status === 'confirmed' || h.status === 'pending').length, color: COLORS.info },
        ].map((s, i) => (
      ...
      {loading ? (
        ...
        <FlatList
          ...
          refreshControl={
            <RefreshControl refreshing={refresh} tintColor={COLORS.gold}
